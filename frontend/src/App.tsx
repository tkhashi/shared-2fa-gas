...
import { TotpPageData, encodeServiceAccountId, TOTP_STEP_SECONDS } from './types';
...
const App: React.FC<AppProps> = ({ initialData }) => {
  const [data] = useState<TotpPageData>(() => JSON.parse(initialData));
  const [remainingSec, setRemainingSec] = useState<number>(data.remaining || TOTP_STEP_SECONDS);

  // トークンが存在する場合のみカウントダウンタイマーを開始
  useEffect(() => {
    // トークンが存在しない場合はタイマーを開始しない
    if (!data.token) {
      return;
    }
